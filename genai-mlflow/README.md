...
**Note:** You can use the provided [Containerfile](Containerfile) to build a custom container image or reference your own. Currently, the `03-mlflow-server.yml` deployment file references the image `quay.io/redhat-et/mlflow:latest` in the `spec.containers.image` section of the Deployment YAML.

A prebuilt image is available at: [https://quay.io/redhat-et/mlflow:latest](https://quay.io/redhat-et/mlflow:latest).

## Deployment procedure

1. **Clone** or navigate to [this repository](https://github.com/redhat-et/mlflow-openshift.git).

    To get started, clone the repository using:

    ```sh
    git clone https://github.com/redhat-et/mlflow-openshift.git
    cd genai-mlflow
    ```

2. In the `openshift` folder, you will find the following YAML files that allow
...
You can run this `oc` command: `oc apply -f ./openshift/.` to execute all of the
above described YAML files located in the **openshift** folder at once.
...
To delete all resources if not necessary just run `oc delete -f ./openshift/.`.
...
# -   RHOAI Workbench (Jupyter Notebooks)
